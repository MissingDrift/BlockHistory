...
import static me.missingdrift.blockhistory.util.ActionColor.CREATE;
import static me.missingdrift.blockhistory.util.ActionColor.DESTROY;
import static me.missingdrift.blockhistory.util.ActionColor.INTERACT;
import static me.missingdrift.blockhistory.util.MessagingUtil.createTextComponentWithColor;
import static me.missingdrift.blockhistory.util.MessagingUtil.prettyDate;
import static me.missingdrift.blockhistory.util.MessagingUtil.prettyLocation;
import static me.missingdrift.blockhistory.util.MessagingUtil.prettyMaterial;
import static me.missingdrift.blockhistory.util.MessagingUtil.prettyState;
    ...
    private String getTypeDetails(BlockData type, byte[] typeState, BlockData oldType, byte[] oldTypeState) {
        ...
        if (BlockStateCodecs.hasCodec(type.getMaterial())) {
            try {
                ...
                blockhistory.getInstance().getLogger().log(Level.SEVERE, "Could not parse BlockState for " + type.getMaterial(), e);
            }
        }
