...
import me.missingdrift.blockhistory.blockstate.BlockStateCodecSign;
import me.missingdrift.blockhistory.config.Config;
import me.missingdrift.blockhistory.config.WorldConfig;
import me.missingdrift.blockhistory.util.ComparableVersion;
import me.missingdrift.blockhistory.util.UUIDFetcher;
...
import static me.missingdrift.blockhistory.config.Config.getLoggedWorlds;
import static me.missingdrift.blockhistory.config.Config.isLogging;
import static me.missingdrift.blockhistory.util.BukkitUtils.friendlyWorldname;
...
class Updater {
    ...
    Updater(blockhistory blockhistory) {
        this.blockhistory = blockhistory;
    }

    boolean update() {
        final ConfigurationSection config = blockhistory.getConfig();
        String versionString = config.getString("version");
        ComparableVersion configVersion = new ComparableVersion(versionString);
        // if (configVersion.compareTo(new ComparableVersion(blockhistory.getDescription().getVersion().replace(" (manually compiled)", ""))) >= 0) {
        // return false;
        // }
        if (configVersion.compareTo(new ComparableVersion("1.2.7")) < 0) {
            blockhistory.getLogger().info("Updating tables to 1.2.7 ...");
            if (isLogging(Logging.CHAT) || isLogging(Logging.PLAYER_COMMANDS) || isLogging(Logging.CONSOLE_COMMANDS) || isLogging(Logging.COMMANDBLOCK_COMMANDS)) {
                final Connection conn = blockhistory.getConnection();
                try {
                    ...
                    blockhistory.getLogger().log(Level.SEVERE, "[Updater] Error: ", ex);
                    return false;
                }
            }
        ...
        if (configVersion.compareTo(new ComparableVersion("1.3")) < 0) {
            blockhistory.getLogger().info("Updating config to 1.3.0 ...");
        ...
        if (configVersion.compareTo(new ComparableVersion("1.3.1")) < 0) {
            blockhistory.getLogger().info("Updating tables to 1.3.1 ...");
            final Connection conn = blockhistory.getConnection();
            try {
                conn.setAutoCommit(true);
                final Statement st = conn.createStatement();
        ...
        if (configVersion.compareTo(new ComparableVersion("1.3.2")) < 0) {
            blockhistory.getLogger().info("Updating tables to 1.3.2 ...");
            final Connection conn = blockhistory.getConnection();
            try {
                conn.setAutoCommit(true);
                final Statement st = conn.createStatement();
        ...
        if (configVersion.compareTo(new ComparableVersion("1.4")) < 0) {
            blockhistory.getLogger().info("Updating config to 1.4.0 ...");
            config.set("clearlog.keepLogDays", null);
            config.set("version", "1.4.0");
        }
        if (configVersion.compareTo(new ComparableVersion("1.4.2")) < 0) {
            blockhistory.getLogger().info("Updating config to 1.4.2 ...");
            for (final String world : config.getStringList("loggedWorlds")) {
                final File file = new File(blockhistory.getDataFolder(), friendlyWorldname(world) + ".yml");
                final YamlConfiguration wcfg = YamlConfiguration.loadConfiguration(file);
                if (wcfg.contains("blockhistoryCreations")) {
                    wcfg.set("logging.BLOCKPLACE", wcfg.getBoolean("blockhistoryCreations"));
                }
                if (wcfg.contains("blockhistoryDestroyings")) {
                    wcfg.set("logging.BLOCKBREAK", wcfg.getBoolean("blockhistoryDestroyings"));
                }
                ...
                wcfg.set("blockhistoryCreations", null);
                wcfg.set("blockhistoryDestroyings", null);
        ...
        if (configVersion.compareTo(new ComparableVersion("1.5.1")) < 0) {
            blockhistory.getLogger().info("Updating tables to 1.5.1 ...");
            final Connection conn = blockhistory.getConnection();
            try {
                conn.setAutoCommit(true);
                final Statement st = conn.createStatement();
        ...
        if (configVersion.compareTo(new ComparableVersion("1.5.2")) < 0) {
            blockhistory.getLogger().info("Updating tables to 1.5.2 ...");
            final Connection conn = blockhistory.getConnection();
            try {
                conn.setAutoCommit(true);
                final Statement st = conn.createStatement();
                ...
                if (rs.next() && rs.getString("Type").equalsIgnoreCase("time")) {
                    ...
                    blockhistory.getLogger().info("Column lb-players was already modified, skipping it.");
                }
                st.close();
                conn.close();
            } catch (final SQLException ex) {
        ...
        if (configVersion.compareTo(new ComparableVersion("1.8.1")) < 0) {
            blockhistory.getLogger().info("Updating tables to 1.8.1 ...");
            final Connection conn = blockhistory.getConnection();
            try {
                conn.setAutoCommit(true);
                final Statement st = conn.createStatement();
                for (final WorldConfig wcfg : getLoggedWorlds()) {
                    if (wcfg.isLogging(Logging.CHESTACCESS)) {
                        ...
                        blockhistory.getLogger().info("Table " + wcfg.table + "-chest modified");
                    }
                }
                st.close();
                conn.close();
        ...
        if (configVersion.compareTo(new ComparableVersion("1.9")) < 0) {
            blockhistory.getLogger().info("Updating tables to 1.9.0 ...");
            blockhistory.getLogger().info("Importing UUIDs for large databases may take some time");
            final Connection conn = blockhistory.getConnection();
            try {
                conn.setAutoCommit(true);
                final Statement st = conn.createStatement();
            ...
            try {
                ...
                blockhistory.getLogger().info(total + " players to convert");
                int done = 0;
                    ...
                    if (names.size() > 0) {
                        ...
                        for (Map.Entry<String, Integer> entry : players.entrySet()) {
                            if (response.get(entry.getKey()) == null) {
                                ...
                                blockhistory.getLogger().warning(entry.getKey() + " not found - giving UUID of " + theUUID);
                            } else {
                        ...
                        players.clear();
                        names.clear();
                        blockhistory.getLogger().info("Processed " + Integer.toString(done) + " out of " + total);
                        rs.close();
                        rs = st.executeQuery("SELECT playerid,playername FROM `lb-players` WHERE LENGTH(UUID)=0 LIMIT " + Integer.toString(UUID_CONVERT_BATCH_SIZE));
                    }
                }
                ...
                blockhistory.getLogger().log(Level.SEVERE, "[UUID importer]", ex);
                return false;
            }
            config.set("version", "1.9.0");
        }
        if (configVersion.compareTo(new ComparableVersion("1.9.4")) < 0) {
            blockhistory.getLogger().info("Updating tables to 1.9.4 ...");
            final Connection conn = blockhistory.getConnection();
            try {
                conn.setAutoCommit(true);
                final Statement st = conn.createStatement();
        ...
        if (configVersion.compareTo(new ComparableVersion("1.10.0")) < 0) {
            blockhistory.getLogger().info("Updating tables to 1.10.0 ...");
            final Connection conn = blockhistory.getConnection();
            try {
                conn.setAutoCommit(true);
                final Statement st = conn.createStatement();
        ...
        if (configVersion.compareTo(new ComparableVersion("1.12.0")) < 0) {
            blockhistory.getLogger().info("Updating tables to 1.12.0 ...");
            if (isLogging(Logging.CHAT) || isLogging(Logging.PLAYER_COMMANDS) || isLogging(Logging.CONSOLE_COMMANDS) || isLogging(Logging.COMMANDBLOCK_COMMANDS)) {
                final Connection conn = blockhistory.getConnection();
                try {
                    conn.setAutoCommit(true);
        ...
        if (configVersion.compareTo(new ComparableVersion("1.13.0")) < 0) {
            blockhistory.getLogger().info("Updating tables to 1.13.0 ...");
            try {
                MaterialUpdater1_13 materialUpdater = new MaterialUpdater1_13(blockhistory);
                blockhistory.getLogger().info("Convertig BlockId to BlockData. This can take a while ...");
                final Connection conn = blockhistory.getConnection();
                conn.setAutoCommit(false);
                final Statement st = conn.createStatement();
                for (final WorldConfig wcfg : getLoggedWorlds()) {
                    blockhistory.getLogger().info("Processing world " + wcfg.world + "...");
                    blockhistory.getLogger().info("Processing block changes...");
                    boolean hadRow = true;
                    ...
                    try {
                        ...
                        if (rs.next()) {
                            ...
                            blockhistory.getLogger().info("Converting " + rowsToConvert + " entries in " + wcfg.table);
                        }
                        rs.close();
                        ...
                        while (hadRow) {
                            ...
                            while (entries.next()) {
                                ...
                                try {
                                    ...
                                    blockhistory.getLogger().info("Exception in entry " + id + " (" + replaced + ":" + data + "->" + type + ":" + data + "): " + e.getMessage());
                                }
                            ...
                            conn.commit();
                            blockhistory.getLogger().info("Done: " + done + "/" + rowsToConvert + " " + (failedRows > 0 ? "Duplicates: " + failedRows + " " : "") + "(" + (rowsToConvert > 0 ? (done * 100 / rowsToConvert) : 100) + "%)");
                        }
                        insertStatement.close();
                        deleteStatement.close();
                    } catch (SQLException e) {
                        blockhistory.getLogger().info("Could not convert " + wcfg.table + ": " + e.getMessage());
                    }

                    blockhistory.getLogger().info("Processing chests...");
                    rowsToConvert = 0;
                    done = 0;
                    try {
                        ...
                        if (rs.next()) {
                            rowsToConvert = rs.getLong(1);
                            blockhistory.getLogger().info("Converting " + rowsToConvert + " entries in " + wcfg.table + "-chest");
                        }
                        rs.close();
                        ...
                        blockhistory.getLogger().info("Could not convert " + wcfg.table + "-chest: " + e.getMessage());
                    }

                    if (wcfg.isLogging(Logging.KILL)) {
                        blockhistory.getLogger().info("Processing kills...");
                        rowsToConvert = 0;
                        done = 0;
                        try {
                            ...
                            if (rs.next()) {
                                rowsToConvert = rs.getLong(1);
                                blockhistory.getLogger().info("Converting " + rowsToConvert + " entries in " + wcfg.table + "-kills");
                            }
                            rs.close();
                            ...
                            for (int start = 0;; start += OTHER_CONVERT_BATCH_SIZE) {
                                ...
                                blockhistory.getLogger().info("Done: " + done + "/" + rowsToConvert + " (" + (rowsToConvert > 0 ? (done * 100 / rowsToConvert) : 100) + "%)");
                                if (!anyRow) {
                                    break;
                                }
                            }
                            ...
                            blockhistory.getLogger().info("Could not convert " + wcfg.table + "-kills: " + e.getMessage());
                        }
                    }
                }
                st.close();
                ...
                blockhistory.getLogger().info("Updating config to 1.13.0 ...");
        ...
        if (configVersion.compareTo(new ComparableVersion("1.13.1")) < 0) {
            blockhistory.getLogger().info("Updating tables to 1.13.1 ...");
            try (Connection conn = blockhistory.getConnection()) {
                conn.setAutoCommit(false);
                final Statement st = conn.createStatement();
                for (final WorldConfig wcfg : getLoggedWorlds()) {
                    blockhistory.getLogger().info("Processing world " + wcfg.world + "...");
                    ...
                    if (wcfg.isLogging(Logging.SIGNTEXT)) {
                        ...
                        try {
                            ...
                            if (rs.next()) {
                                rowsToConvert = rs.getLong(1);
                                blockhistory.getLogger().info("Converting " + rowsToConvert + " entries in " + wcfg.table + "-sign");
                            }
                            rs.close();
                            ...
                            blockhistory.getLogger().info("Could not convert " + wcfg.table + "-sign: " + e.getMessage());
                        }
                    }
                }
        ...
        if (configVersion.compareTo(new ComparableVersion("1.16.0")) < 0) {
            blockhistory.getLogger().info("Updating tables to 1.16.0 ...");
            try (Connection conn = blockhistory.getConnection()) {
                conn.setAutoCommit(true);
                final Statement st = conn.createStatement();
                for (final WorldConfig wcfg : getLoggedWorlds()) {
                ...
                blockhistory.getLogger().log(Level.SEVERE, "[Updater] Warning: Could not add index", ex);
            }
            config.set("version", "1.16.0");
        }
        if (configVersion.compareTo(new ComparableVersion("1.17.0")) < 0) {
            blockhistory.getLogger().info("Updating tables to 1.17.0 ...");
            blockhistory.getLogger().warning("The updating process might take several minutes if you have a huge log table! Please do not shutdown your server until it is completed.");
            try (Connection conn = blockhistory.getConnection()) {
                conn.setAutoCommit(true);
                final Statement st = conn.createStatement();
                for (final WorldConfig wcfg : getLoggedWorlds()) {
                ...
                blockhistory.getLogger().log(Level.SEVERE, "[Updater] Warning: Could not alter table", ex);
            }
            blockhistory.getLogger().info("Update to 1.17.0 completed.");
            ...
            for (final WorldConfig wcfg : getLoggedWorlds()) {
                ...
                if (type != null) {
                    if (type.contains("tinyint") || type.contains("unsigned")) {
                        blockhistory.getLogger().info("Fixing y column type for table " + wcfg.table + "-blocks ...");
                        blockhistory.getLogger().warning("The updating process might take several minutes if you have a huge log table! Please do not shutdown your server until it is completed.");
                        st.executeUpdate("ALTER TABLE `" + wcfg.table + "-blocks` CHANGE `y` `y` SMALLINT(5) NOT NULL");
                    }
                }
                ...
                if (type != null) {
                    if (type.contains("tinyint") || type.contains("unsigned")) {
                        blockhistory.getLogger().info("Fixing y column type for table " + wcfg.table + "-entities ...");
                        blockhistory.getLogger().warning("The updating process might take several minutes if you have a huge log table! Please do not shutdown your server until it is completed.");
                ...
                if (type != null) {
                    if (type.contains("tinyint") || type.contains("unsigned")) {
                        blockhistory.getLogger().info("Fixing y column type for table " + wcfg.table + "-kills ...");
                        blockhistory.getLogger().warning("The updating process might take several minutes if you have a huge log table! Please do not shutdown your server until it is completed.");
    ...
    void createIndexIfDoesNotExist(String table, String indexName, String definition, Statement st, boolean silent) throws SQLException {
        ...
        if (!rs.next()) {
            ...
            blockhistory.getLogger().info("Add index " + indexName + " to table " + table + ": Table modified");
        } else if (!silent) {
            blockhistory.getLogger().info("Add index " + indexName + " to table " + table + ": Already fine, skipping it");
        }
        rs.close();
    }
    ...
    void checkCharset(String table, String column, Statement st, boolean silent) throws SQLException {
        ...
        if (rs.next() && !rs.getString("Collation").substring(0, charset.length()).equalsIgnoreCase(charset)) {
            ...
            blockhistory.getLogger().info("Table " + table + " modified");
        } else if (!silent) {
            blockhistory.getLogger().info("Table " + table + " already fine, skipping it");
        }
        rs.close();
    }
    ...
    private void createTable(Statement state, String table, String query) throws SQLException {
        try (ResultSet tableResult = state.executeQuery("SHOW TABLES LIKE '" + table + "'")) {
            if (!tableResult.next()) {
                blockhistory.getLogger().log(Level.INFO, "Creating table " + table + ".");
    ...
    private void updateMaterialsPost1_13() {
        ...
        String currentMinecraftVersion = blockhistory.getServer().getVersion();
        ...
        blockhistory.getLogger().info("[Updater] Current Minecraft Version: '" + currentMinecraftVersion + "'");
        ...
        if (comparablePreviousMinecraftVersion.compareTo("1.14") < 0 && comparableCurrentMinecraftVersion.compareTo("1.14") >= 0) {
            blockhistory.getLogger().info("[Updater] Upgrading Materials to 1.14");
        ...
        if (comparablePreviousMinecraftVersion.compareTo("1.17") < 0 && comparableCurrentMinecraftVersion.compareTo("1.17") >= 0) {
            blockhistory.getLogger().info("[Updater] Upgrading Materials to 1.17");
    ...
    private void renameMaterial(String oldName, Material newName) {
        ...
        try {
            ...
            if (rs.next()) {
                blockhistory.getLogger().info("[Updater] Updating " + oldName + " to " + newName);
                ...
                st.close();
                if (rows > 0) {
                    blockhistory.getLogger().info("[Updater] Successfully updated " + rows + " entries..");
                }
            }
            ...
            blockhistory.getLogger().log(Level.SEVERE, "[Updater] Error: " + ex.getMessage(), ex);
        }
    }
    ...
    public static class PlayerCountChecker implements Runnable {
        ...
        public PlayerCountChecker(blockhistory blockhistory) {
            this.blockhistory = blockhistory;
        }
        ...
        public void run() {
            ...
            try {
                ...
                if (rs.next()) {
                    for (int i = 0; i < 6; i++) {
                        blockhistory.getLogger().warning("Your server reached 65000 players. You should soon update your database table schema - see FAQ: https://github.com/blockhistory/blockhistory/wiki/FAQ#blockhistory-your-server-reached-65000-players-");
                    }
                }
                st.close();
                conn.close();
    ...
    public static class MaterialUpdater1_13 {
        ...
        public MaterialUpdater1_13(blockhistory plugin) throws IOException {
