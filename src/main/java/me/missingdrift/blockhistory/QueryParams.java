...
import me.missingdrift.blockhistory.util.CuboidRegion;
import me.missingdrift.blockhistory.util.SqlUtil;
...
import static me.missingdrift.blockhistory.util.Utils.*;
...
public final class QueryParams implements Cloneable {
    ...
    public QueryParams(blockhistory blockhistory) {
        this.blockhistory = blockhistory;
    }

    public QueryParams(blockhistory blockhistory, CommandSender sender, List<String> args) throws IllegalArgumentException {
        this.blockhistory = blockhistory;
            ...
            if (param.equals("last")) {
                ...
                for (String playerName : values) {
                    if (playerName.length() > 0) {
                        ...
                        if (playerName.contains("\"")) {
                            ...
                            final Player matches = blockhistory.getServer().getPlayerExact(playerName);
                ...
                for (final String killerName : values) {
                    if (killerName.length() > 0) {
                        ...
                        if (killerName.contains("\"")) {
                            ...
                            final List<Player> matches = blockhistory.getServer().matchPlayer(killerName);
                            if (matches.size() > 1) {
                ...
                for (final String victimName : values) {
                    if (victimName.length() > 0) {
                        ...
                        if (victimName.contains("\"")) {
                            ...
                            final List<Player> matches = blockhistory.getServer().matchPlayer(victimName);
                            if (matches.size() > 1) {
                                throw new IllegalArgumentException("Ambiguous victimname '" + param + "'");
                            }
                ...
                for (String blockName : values) {
                    ...
                    if (blockName.startsWith("#")) {
                        ...
                        Tag<Material> tag = blockhistory.getServer().getTag(Tag.REGISTRY_BLOCKS, NamespacedKey.minecraft(tagName), Material.class);
                        if (tag == null || tag.getValues().isEmpty()) {
                            tag = blockhistory.getServer().getTag(Tag.REGISTRY_ITEMS, NamespacedKey.minecraft(tagName), Material.class);
                            if (tag == null || tag.getValues().isEmpty()) {
