...
import static me.missingdrift.blockhistory.config.Config.dontRollback;
import static me.missingdrift.blockhistory.config.Config.replaceAnyway;
import static me.missingdrift.blockhistory.util.BukkitUtils.*;
...
public class WorldEditor implements Runnable {
    ...
    public WorldEditor(blockhistory blockhistory, World world) {
        this(blockhistory, world, false);
    }

    public WorldEditor(blockhistory blockhistory, World world, boolean forceReplace) {
    ...
    synchronized public void start() throws Exception {
        ...
        taskID = blockhistory.getServer().getScheduler().scheduleSyncRepeatingTask(blockhistory, this, 0, 1);
        if (taskID == -1) {
    ...
    public synchronized void run() {
        ...
        while (!edits.isEmpty() && counter < 100) {
            try {
                ...
                blockhistory.getLogger().log(Level.WARNING, "[WorldEditor] Exeption: ", ex);
            }
            counter++;
            if (sender != null) {
                float percentage = ((size - edits.size()) / size) * 100.0F;
                if (percentage % 20 == 0) {
                    sender.sendMessage(ChatColor.GOLD + "[blockhistory]" + ChatColor.YELLOW + " Rollback progress: " + percentage + "%" +
                            " Blocks edited: " + counter);
                }
            }
        }
        if (edits.isEmpty()) {
            blockhistory.getServer().getScheduler().cancelTask(taskID);
            if (errorList.size() > 0) {
                try {
                    final File file = new File("plugins/blockhistory/error/WorldEditor-" + new SimpleDateFormat("yy-MM-dd-HH-mm-ss").format(System.currentTimeMillis()) + ".log");
            ...
            if (changeType == (rollback ? EntityChangeType.KILL : EntityChangeType.CREATE)) {
                ...
                if (result == null) {
                    ...
                    if (!result.getUniqueId().equals(uuid)) {
                        blockhistory.getConsumer().queueEntityUUIDChange(world, entityId, result.getUniqueId());
